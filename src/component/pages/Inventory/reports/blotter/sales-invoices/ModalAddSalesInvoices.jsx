...
const ModalAddSalesInvoices = ({ item }) => {
  ...
  const mutation = useMutation({
    mutationFn: (values) =>
      queryData(
        item
          ? `/v1/report-sales-invoices/${item.or_invoice_aid}`
          : `/v1/report-sales-invoices`,
        item ? "put" : "post",
        values
      ),
    onSuccess: (data) => {
      // Invalidate and refetch
      queryClient.invalidateQueries({ queryKey: ["blotter-sales-invoices"] });
      // show success box
      if (data.success) {
  ...
  const { data: readAllMemberSI, isLoading } = useQueryData(
    `/v1/report-sales-invoices/read-all-member`, // endpoint
    "get", // method
    "readAllMemberSI" // key
  );
  const initVal = {
                  ...
                  <Form>
                    ...
                    <div className="relative ">
                      ...
                      >
                        ...
                        {readAllMemberSI?.data.map((mItem, key) => {
                          return (
...
export default ModalAddSalesInvoices;
