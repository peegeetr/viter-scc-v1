...
const ModalAddBeneficiaries = ({ item }) => {
  ...
  const mutation = useMutation({
    mutationFn: (values) =>
      queryData( 
        item ? `/v1/beneficiaries/${item.beneficiaries_aid}` : `/v1/beneficiaries`,
        item ? "put" : "post", 
        values
      ),
    onSuccess: (data) => {
  ...
  const initVal = { 
    ...
    beneficiaries_employee_id: item ? item.beneficiaries_employee_id : "",
    ...
    >
      <div className="p-1 w-[350px] rounded-b-2xl animate-slideUp ">
        <div className="flex justify-between items-center bg-primary p-3 rounded-t-2xl">
          <h3 className="text-white text-sm">Update Additional Information
          </h3>
          <button
            type="button"
